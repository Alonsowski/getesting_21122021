...
class SaleApprovalReason(models.TransientModel):
    ...
    ('for_amount', 'Solicitud por Monto'),
    ('for_discount', 'Solicitud por Descuento'),
    ], 'Tipo de Aprobación', copy=False, required=True)
    requested_discount = fields.Float('Porcentaje de Descuento')
    ...
    def approve(self):
        ...
        if self.approval_for == 'for_amount':
            ...
            if user_search:
                ...
                raise UserError('No existe un aprobador para este limite. Contacte a su administrador')
